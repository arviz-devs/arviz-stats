...
class TestCauchyCombination:
    ...
    def test_truncated(self):
        ...
        cauchy_vals = np.tan((0.5 - ps) * np.pi)
        mask = (ps < 0.5).astype(float)
        truncated_mean = np.mean(cauchy_vals * mask)
...
class TestUniformityTestPotC:
    def test_basic_values(self):
        """For sorted x = [0.2, 0.8], n=2:

        Beta(1,2).cdf(0.2) = 1 - 0.8^2 = 0.36
        Beta(2,1).cdf(0.8) = 0.8^2 = 0.64
        ps = 2*min(cdf, 1-cdf) = [0.72, 0.72]
        """
        x = np.array([0.8, 0.2])
...
class TestUniformityTestPritC:
    def test_basic_values(self):
        ...
        sorted x = [0.5, 0.5]
        ranks = searchsorted(side='right') = [2, 2]
        probs1 = binom.cdf(2, 2, 0.5) = 1.0
        probs2 = binom.cdf(1, 2, 0.5) = 0.75
        ps = 2 * min(1.0, 1 - 0.75) = 2 * 0.25 = 0.5
        """
        x = np.array([0.5, 0.5])
