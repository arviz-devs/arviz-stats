...
def test_compare_different_size(centered_eight, non_centered_eight):
    ...
    centered_eight = centered_eight.filter(
        lambda node: "stats" not in str(node.name)
    ).map_over_subtree(lambda node: node.drop_sel(school="Choate"))
...
def test_calculate_ics_ic_override(centered_eight, non_centered_eight):
    ...
    assert ic == "waic"
    assert out_dict["centered"]["elpd"] == waic(centered_eight)["elpd"]
...
def test_summary_groups(centered_eight, missing_groups):
    if missing_groups == "posterior":
        centered_eight = deepcopy(centered_eight).drop_nodes("posterior")
    elif missing_groups == "prior":
        centered_eight = deepcopy(centered_eight).drop_nodes(("posterior", "prior"))
...
def test_summary_wrong_group(centered_eight):
    with pytest.raises(TypeError, match=r"DataTree does not contain group: InvalidGroup"):
...
def test_summary_labels():
    ...
    data = from_dict(
        {"posterior": {"a": np.random.randn(4, 100, 4, 5)}},
...
def test_waic(centered_eight, multidim_models, scale, multidim):
    ...
    assert waic_pointwise.kind == "waic"
    assert waic_pointwise.elpd_i is not None
    assert waic_pointwise.pareto_k is None


def test_waic_bad(centered_eight):
    """Test widely available information criterion calculation"""
    centered_eight = deepcopy(centered_eight).drop_nodes("log_likelihood")
...
def test_loo(centered_eight, multidim_models, scale, multidim):
    ...
    assert loo_pointwise.kind == "loo"
    assert loo_pointwise.elpd_i is not None
    assert loo_pointwise.pareto_k is not None


def test_loo_one_chain(centered_eight):
    centered_eight = deepcopy(centered_eight)
    centered_eight.posterior = centered_eight.posterior.drop_sel(chain=[1, 2, 3])
    centered_eight.sample_stats = centered_eight.sample_stats.drop_sel(chain=[1, 2, 3])
...
def test_multidimensional_log_likelihood(func):
    ...
    statsm = Dataset({"y": DataArray(llm, dims=["chain", "draw", "a", "b"])})

    stats1 = Dataset({"y": DataArray(ll1, dims=["chain", "draw", "v"])})

    post = Dataset({"mu": DataArray(np.random.rand(4, 23, 2), dims=["chain", "draw", "v"])})

    dsm = DataTree.from_dict({"log_likelihood": statsm, "posterior": post})
    ds1 = DataTree.from_dict({"log_likelihood": stats1, "posterior": post})
...
def test_loo_pit_bad_input(centered_eight, input_type):
    ...
    if input_type == "idataarray":
        with pytest.raises(ValueError, match=r"type DataTree or None"):
...
def test_weight_predictions():
    ...
    idata0 = from_dict(
        {"posterior_predictive": {"a": rng.normal(-1, 1, (1, 1000))}, "observed_data": {"a": 1}},
    )
    idata1 = from_dict(
        {"posterior_predictive": {"a": rng.normal(1, 1, (1, 1000))}, "observed_data": {"a": 1}},
    )
