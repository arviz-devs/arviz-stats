...
class TestFFT:
    ...
    def test_rfft_axis(self, core, axis):
        data = np.random.randn(4, 10, 5)
...
class TestAutoCorrelation:
    ...
    def test_autocov_shape(self, core, axis):
        x = np.random.randn(10, 20, 5)
        result = core.autocov(x, axis=axis)
        assert result.shape == x.shape

    def test_autocov_lag_zero(self, core):
...
class TestQuantile:
    ...
    def test_quantile_axis(self, core, axis):
...
class TestRanking:
    ...
    def test_float_rankdata_range(self, core):
...
class TestBinning:
    def test_get_bininfo(self, core):
        x = np.random.randn(100)
    ...
    @pytest.mark.parametrize("bins", [10, 20, 50])
    def test_histogram_bins(self, core, bins):
        x = np.random.randn(1000)
        counts, _ = core._histogram(x, bins=bins)
        assert len(counts) == bins

    def test_histogram_density(self, core):
        x = np.random.randn(1000)
    ...
    def test_histogram_range(self, core):
        x = np.random.randn(1000)
...
class TestHDI:
    ...
    def test_hdi_nearest(self, core):
        x = np.random.randn(1000)
    ...
    @pytest.mark.parametrize("prob", [0.5, 0.9, 0.95])
    def test_hdi_nearest_circular(self, core, prob):
        x = np.random.vonmises(0, 2, size=1000)
        result = core._hdi_nearest(x, prob=prob, circular=True, skipna=False)
        assert result.shape == (2,)
        assert np.all(result >= -np.pi)
        assert np.all(result <= np.pi)

    def test_hdi_nearest_skipna(self, core):
        x = np.concatenate([np.random.randn(100), [np.nan] * 10])
...
class TestMode:
    ...
    def test_mode_continuous(self, core):
