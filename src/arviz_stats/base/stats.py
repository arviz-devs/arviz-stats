...
from arviz_base.rcparams import ScaleKeyword, ICKeyword
from arviz_base.utils import _var_names, _get_coords
...
def hdi(
    ...
    **dask_kwargs,
    ...
    if coords is not None:
        ary = _get_coords(ary, coords)
    var_names = _var_names(var_names, ary, filter_vars)
    ary = ary[var_names] if var_names else ary

    hdi_coord = xr.DataArray(["lower", "higher"], dims=["hdi"], attrs={"hdi_prob": hdi_prob})
    hdi_data = _wrap_xarray_ufunc(
        func, ary, func_kwargs=func_kwargs, **dask_kwargs
